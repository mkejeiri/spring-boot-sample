...
//		tacosRecipe.getIngredients().add(new Ingredient("lime, cut into wedges", new BigDecimal(4), eachUom, tacosRecipe));
	...
	collection need to be initialised within the transaction within the same Hibernate
	session, the PB is the exception get thrown because we getting it outside of that. 
	By adding @Transactional we can get around of this issue: we direct spring framework to create a transaction around
	that onApplicationEvent method, so all getters and setters of the object graph run within the context/transaction boundaries!	
	 * */ 
	@Override
